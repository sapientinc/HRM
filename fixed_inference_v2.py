...
def run_actual_inference(model, model_config, tokenizer, prompt):
    ...
    try:
        with torch.no_grad():
            ...
            # Initialize proper carry structure ONCE
            carry = model.initial_carry(batch)
            
            print("✅ Carry structure initialized properly")
            
            # Run forward pass through HRM with proper carry. The `carry` object is now updated.
            ...
            # Try to generate a few more tokens with the corrected stateful logic
            print(f"\n🔄 Generating longer completion with corrected stateful logic...")
...
def main():
    print("🧠 HRM Language Model - REAL Inference Test (V2 - Corrected Carry Logic)")
    print("=" * 60)
